...
description: Advertise one GB10 GPU without replacing the pinned host driver or toolkit.
---
...
Version 26.7.0 is pinned from the released NVIDIA chart and meets its documented Kubernetes minimum, but this complete k3s/GB10 combination is not end-to-end validated here. The accepted production workload uses a different kubeadm cluster.
:::
...
## Require one allocatable GPU on each GB10

---
title: From bare DGX Spark hosts to routed DeepSeek
...
:::note[What I run]
I run the image and two-node LeaderWorkerSet on a kubeadm/Cilium cluster. This tutorial adapts that setup to k3s. I have not run the complete k3s path on two spare GB10 hosts; the CPU-only controller checks do not exercise GPU or RDMA traffic.
:::
...
Perform the [64 KiB kernel and driver procedure](/vllm-multiarch-oci/guides/gb10-64k-driver/) on **each GPU host**. The gate is not merely `nvidia-smi`: require kernel `7.0.0-1019-nvidia-64k`, page size `65536`, all five module artifacts at 615.71.09, and a CUDA allocation/synchronization test.
...
Render the routing bundle and follow [request routing with EPP and Envoy AI Gateway](/vllm-multiarch-oci/guides/llm-d-routing/).
